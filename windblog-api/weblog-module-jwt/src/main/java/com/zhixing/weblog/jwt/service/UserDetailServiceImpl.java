package com.zhixing.weblog.jwt.service;
...
import com.zhixing.weblog.comm.domain.mapper.UserRoleMapper;
...
public class UserDetailServiceImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        if (!CollectionUtils.isEmpty(roleDOS)) {
            List<String> roles = roleDOS.stream().map(role -> role.getRole()).collect(Collectors.toList());
            roleArr = roles.toArray(new String[roles.size()]);
        }
        // 设置用户验证信息，authorities 指定角色
